...
@then(u'owner text post - go to community')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.comm_north_tab).click()
    sleep(2)


@then(u'owner text post - make a new text post as owner')
def step_impl(context):
    sleep(5)
...
@then(u'owner text post - validating text post passed')
def step_impl(context):
...
@then(u'owner text post - deleting post')
def step_impl(context):
...
@when(u'owner text post - user logout')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.timeline_tab).send_keys(Keys.UP)
    sleep(3)
...
@then(u'owner text post - direct to landing page')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.landing)
