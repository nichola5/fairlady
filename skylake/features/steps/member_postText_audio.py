...
@given(u'member text audio Post - opening sebangsa')
def step_impl(context):
    #context.browser.get(database.userdata["url"])
    context.browser.find_element(By.XPATH,locator.landing)

@when(u'member text audio Post - login using bot_one as member')
def step_impl(context):
...
@then(u'member text audio Post - go to community')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.comm_north_tab).click()
    sleep(2)


@then(u'member text audio Post - make a new text post with audio as member')
def step_impl(context):
...
@then(u'member text audio Post - validating text post passed')
def step_impl(context):
...
@then(u'member text audio Post - validating audio passed')
def step_impl(context):
...
@then(u'member text audio Post - deleting post')
def step_impl(context):
    ...
    sleep(1)
    context.browser.find_element(By.XPATH,locator.comm_post_delete_mbr).click()
    sleep(2)
...
@when(u'member text audio Post - user logout')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.timeline_tab).send_keys(Keys.UP)
    sleep(3)
...
@then(u'member text audio Post - direct to landing page')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.landing)
