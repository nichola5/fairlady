...
@given(u'owner text IMG Post - opening sebangsa')
def step_impl(context):
    #context.browser.get(database.userdata["url"])
    context.browser.find_element(By.XPATH,locator.landing)

@when(u'owner text IMG Post - login using bot_one as owner')
def step_impl(context):
...
@then(u'owner text IMG Post - go to community')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.comm_north_tab).click()
    sleep(2)


@then(u'owner text IMG Post - make a new text post with images as owner')
def step_impl(context):
...
@then(u'owner text IMG Post - validating text post passed')
def step_impl(context):
...
@then(u'owner text IMG Post - validating images passed')
def step_impl(context):
...
@then(u'owner text IMG Post - deleting post')
def step_impl(context):
...
@when(u'owner text IMG Post - user logout')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.timeline_tab).send_keys(Keys.UP)
    sleep(3)
...
@then(u'owner text IMG Post - direct to landing page')
def step_impl(context):
    context.browser.find_element(By.XPATH,locator.landing)
